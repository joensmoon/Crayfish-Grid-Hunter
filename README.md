# Crayfish Grid Hunter (小龙虾网格猎人) v4.0.0
...
Crayfish Grid Hunter 不是一个简单的技术指标计算器，而是一个具备**多维度智能决策能力**的网格交易专家。

| 功能模块 | 核心逻辑 | 集成的官方 Skill |
| :--- | :--- | :--- |
| **智能筛选** | 扫描过去 14 天波动率高且趋势平稳（横盘）的币种 | `crypto-market-rank` + `spot` |
| **动态区间** | 基于布林带和 72 小时支撑/压力位生成网格区间 | `spot` (Klines API) |
| **聪明钱验证** | 交叉验证候选币种是否有 Smart Money 买入信号 | `trading-signal` |
| **安全审计** | 推荐前自动检测蜜罐、Rug Pull、异常税率等风险 | `query-token-audit` |
| **费用优化** | 检查并建议开启 BNB 抵扣手续费，最大化网格收益 | `assets` |
| **突破预警** | 持续监控价格是否接近区间边缘，伴随量能异动时预警 | `spot` (Ticker API) |
...
Step 4: 安全审计 ──── query-token-audit (蜜罐/Rug Pull/税率检测)
    ...
    ▼
Step 5: 费用优化 ──── assets (余额检查 + BNB燃烧抵扣)
    │
    ▼
...
├── test_grid_hunter.py                   # 完整测试脚本（9个测试用例）
...
```bash
npx skills add https://github.com/joensmoon/123 -a openclaw -y
```
