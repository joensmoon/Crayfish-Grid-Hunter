...
*   **智能筛选**：扫描过去 30 天波动率高且趋势平稳（横盘）的币种，使用标准 Wilder 平滑算法计算 RSI。
*   **动态区间**：基于标准 20 期布林带（Bollinger Bands）和 72 小时支撑/压力位生成网格区间。
*   **聪明钱验证**：交叉验证候选币种是否有 Smart Money 买入信号。
*   **安全审计**：推荐前自动检测 BSC 链上代币的蜜罐、Rug Pull、异常税率等风险。
...
├── test_grid_hunter.py                   # 完整测试脚本（包含RSI/BB算法验证）
...
Crayfish Grid Hunter 集成了 5 个币安官方 Skill。运行以下命令一次性安装全部依赖（注意：此命令会将 `binance-skills-hub` 仓库中所有的 skills 下载到本地环境，您也可以选择性安装）：

```bash
npx skills add https://github.com/binance/binance-skills-hub -a openclaw -y
```
